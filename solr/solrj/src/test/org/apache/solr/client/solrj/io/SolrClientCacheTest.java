...
public class SolrClientCacheTest extends SolrCloudTestCase {
   ...
   * suite. That ordering is upstream's, unchanged at the releases that carry the fix, so
   * closing it would mean writing production code upstream did not write; the test cannot
   * close it either, because the client is unreachable by the time expectThrows returns.
   * Clearing here is what SolrTestCaseJ4.teardownTestCases itself does in its finally block,
   * one @AfterClass later.
   */
  @AfterClass
  ...
  /**
   * The CVE probe, and the only test here that touches no method this fix adds. Nothing on this
   * release line calls setDefaultZKHost -- CoreContainer has no SolrClientCache at this
   * baseline, where StreamHandler owns a static one -- so a cache with no default host is what
   * a consumer actually gets, and it is the state in which the flaw bites: before the fix the
   * cache hands the local ensemble's ZooKeeper credentials to whatever host a streaming
   * expression names.
   */
  @Test
